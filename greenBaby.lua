...
Section:NewToggle("ToggleBro", "", function(ScrPilot)
    ...
    while GlobScrPilot do
		...
		humanoid = game:GetService("Workspace").Living:WaitForChild(plr.Name):FindFirstChild("Green Baby"):FindFirstChild("Green Baby").Humanoid
		body = game:GetService("Workspace").Living:WaitForChild(plr.Name):FindFirstChild("Green Baby"):FindFirstChild("Green Baby").HumanoidRootPart
...
-- BlackMyxa02
		...
		while Calculate_length(body.Position, destination) < 50 and GlobScrPilot do
			...
			if destination.Y - body.Position.Y > 3 then
				Deb("d")
				game:GetService("Workspace").Living:WaitForChild(plr.Name):FindFirstChild("Green Baby"):FindFirstChild("Green Baby").Humanoid.JumpPower = 
	...
	-- else
		...
		for k, waypoint in pairs(waypoints) do
			...
			if k == 100 or (not GlobScrPilot) then
				print(k, "k")
				Deb("h")
				game:GetService("Workspace").Living:WaitForChild(plr.Name):FindFirstChild("Green Baby"):FindFirstChild("Green Baby").Humanoid.JumpPower = 100
				humanoid.Jump = true
				Deb("h1")
				break
			end
