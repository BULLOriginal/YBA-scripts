...
local GlobTargetNpcPlayerName = "Diavolo"
local GlobNPCControlState
local GlobNPCTeleportState
local GlobNpcWorkPart
...
function InjectToNPC(NpcName, Part)
    if GetPlayer(NpcName):FindFirstChild("HumanoidRootPart")["RotateVictim"] then
        GetPlayer(NpcName):FindFirstChild("HumanoidRootPart")["RotateVictim"]:Destroy()
    end

    if GetPlayer(NpcName):FindFirstChild("HumanoidRootPart")[Part] then
        return GetPlayer(NpcName):FindFirstChild("HumanoidRootPart")[Part]
    else
        GlobNpcWorkPart = Instance.new(Part, GetPlayer(NpcName)[HumanoidRootPart])
        return GetPlayer(NpcName):FindFirstChild("HumanoidRootPart")[Part]
    end
end


function NPCTeleport()
    while(game:GetService("RunService").RenderStepped:wait() and 
    GlobScriptState and GlobNPCControlState and GlobNPCTeleportState) do
		if GetPlayer(GlobTargetNpcName)["HumanoidRootPart"] and GetPlayer(GlobTargetNpcPlayerName)["HumanoidRootPart"] then
			InjectToNPC("BodyPosition", GlobTargetNpcName).Position = GetPlayer(GlobTargetNpcPlayerName)["HumanoidRootPart"].Position
            InjectToNPC("BodyPosition", GlobTargetNpcName).MaxForce = Vector3.new("inf", "inf", "inf")
            wait(0.01)
		end
    end
end
...
MainNPCSection:NewTextBox("Npc Target player Name", "Diavolo as default", function(TargetNpcName)
    GlobTargetNpcPlayerName = TargetNpcPlayerName
	print("New Npc Target player Name is:", NpcName)
end)


MainNPCSection:NewTextBox("Target Npc Name", "Jotaro Kujo as default", function(TargetNpcName)
    GlobTargetNpcName = TargetNpcName
	print("New Target Npc Name is:", GlobTargetNpcName)
end)
...
MainNPCSection:NewToggle("Toggle NPC Teleport to player", "", function(NPCTeleportState)
    GlobNPCTeleportState = NPCTeleportState
    if GlobNPCTeleportState then
        print("GlobNPCJumpingState is On")
        NPCJump(GlobNPCTeleportState)
    end

    if not GlobNPCTeleportState then
        print("GlobNPCJumpingState is Off")
    end
end)
